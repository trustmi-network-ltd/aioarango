...
async def test_document_insert(col: StandardCollection, docs):
    ...
    result = await col.insert(document=doc, overwrite=True, return_old=True, return_new=True)
...
async def test_document_insert_many(col: StandardCollection, bad_col: StandardCollection, docs):
...
async def test_document_update_many(col: StandardCollection, bad_col: StandardCollection, docs):
...
async def test_document_update_match(col: StandardCollection, bad_col: StandardCollection, docs):
    ...
    assert await col.update_match({"val": 3}, {"val": None}, sync=True, keep_none=True) == 1
    assert (await col.get("3"))["val"] is None

    # Test update matching documents with sync and keep_none set to False
    assert await col.update_match({"val": 1}, {"val": None}, sync=False, keep_none=False) == 2
...
async def test_document_replace_many(col: StandardCollection, bad_col: StandardCollection, docs):
    await col.insert_many(docs)
...
async def test_document_replace_match(col: StandardCollection, bad_col: StandardCollection, docs):
    await col.import_bulk(docs)
...
async def test_document_delete_many(col: StandardCollection, bad_col: StandardCollection, docs):
    # Set up test documents
    old_revs = {}
    doc_keys = [d["_key"] for d in docs]
...
async def test_document_delete_match(col: StandardCollection, bad_col: StandardCollection, docs):
    # Set up test documents
    await col.import_bulk(docs)
...
async def test_document_count(col: StandardCollection, bad_col: StandardCollection, docs):
    # Set up test documents
    await col.import_bulk(docs)
...
async def test_document_find(col: StandardCollection, bad_col: StandardCollection, docs):
    # Check preconditions
    assert await col.count() == 0
...
async def test_document_find_near(col: StandardCollection, bad_col: StandardCollection, docs):
    await col.import_bulk(docs)
    ...
    result = [doc async for doc in await col.find_near(latitude=1, longitude=1, limit=0)]
    assert extract("_key", result) == []

    # Test find_near with limit of 1
    result = [doc async for doc in await col.find_near(latitude=1, longitude=1, limit=1)]
    assert extract("_key", result) == ["1"]

    # Test find_near with limit of 3
    result = [doc async for doc in await col.find_near(latitude=1, longitude=1, limit=3)]
    assert extract("_key", result) == ["1", "2", "3"]

    # Test find_near with limit of 3 (another set of coordinates)
    result = [doc async for doc in await col.find_near(latitude=5, longitude=5, limit=3)]
    ...
    result = [doc async for doc in await col.find_near(latitude=1, longitude=1, limit=1)]
    assert list(result) == []
    result = [doc async for doc in await col.find_near(latitude=5, longitude=5, limit=4)]
    assert list(result) == []
...
async def test_document_find_in_range(col: StandardCollection, bad_col: StandardCollection, docs):
    await col.import_bulk(docs)
    ...
    result = [doc async for doc in await col.find_in_range("val", lower=1, upper=2, limit=0)]
    assert extract("_key", result) == []

    # Test find_in_range with limit of 3
    result = [doc async for doc in await col.find_in_range("val", lower=1, upper=5, limit=3)]
    assert extract("_key", result) == ["1", "2", "3"]
    ...
    result = [doc async for doc in await col.find_in_range("val", lower=1, upper=5, skip=0)]
    assert extract("_key", result) == ["1", "2", "3", "4"]

    # Test find_in_range with skip set to 3
    result = [doc async for doc in await col.find_in_range("val", lower=1, upper=5, skip=2)]
...
async def test_document_find_in_radius(col: StandardCollection, bad_col: StandardCollection):
    doc1 = {"_key": "1", "loc": [1, 1]}
    doc2 = {"_key": "2", "loc": [1, 4]}
    doc3 = {"_key": "3", "loc": [4, 1]}
    doc4 = {"_key": "4", "loc": [4, 4]}
    ...
    result = list(
        [doc async for doc in await col.find_in_radius(
            latitude=1,
            longitude=4,
            radius=6,
        )]
...
async def test_document_find_in_box(col: StandardCollection, bad_col: StandardCollection, geo: dict, cluster):
    if cluster:
    ...
    result = [doc async for doc in await col.find_in_box(
...
async def test_document_find_by_text(col: StandardCollection, docs):
    ...
    result = [doc async for doc in await col.find_by_text(field="text", query="foo,|bar")]
    ...
    result = [doc async for doc in await col.find_by_text(field="text", query="foo", limit=0)]
    assert len(list(result)) == 0

    result = [doc async for doc in await col.find_by_text(field="text", query="foo", limit=1)]
    assert len(list(result)) == 1

    result = [doc async for doc in await col.find_by_text(field="text", query="foo", limit=2)]
    assert len(list(result)) == 2

    result = [doc async for doc in await col.find_by_text(field="text", query="foo", limit=3)]
...
async def test_document_get_many(col: StandardCollection, bad_col: StandardCollection, docs):
    # Set up test documents
    await col.import_bulk(docs)
...
async def test_document_keys(col: StandardCollection, bad_col: StandardCollection, docs):
...
async def test_document_export(col: StandardCollection, bad_col: StandardCollection, docs, cluster):
    if cluster:
        pytest.skip("Not tested in a cluster setup")
...
async def test_document_random(col: StandardCollection, bad_col: StandardCollection, docs):
    # Set up test documents
    await col.import_bulk(docs)
...
async def test_document_import_bulk(col: StandardCollection, bad_col: StandardCollection, docs):
    ...
    result = await col.import_bulk([new_doc], on_duplicate="ignore", halt_on_error=False)
    assert await col.count() == 1
    assert result["created"] == 0
    assert result["errors"] == 0
    assert result["empty"] == 0
    ...
    result = await col.import_bulk([new_doc], on_duplicate="update", halt_on_error=False)
    assert await col.count() == 1
    assert result["created"] == 0
    assert result["errors"] == 0
    assert result["empty"] == 0
    ...
    await empty_collection(col)
    await col.insert(old_doc)
    result = await col.import_bulk([new_doc], on_duplicate="replace", halt_on_error=False)
    assert await col.count() == 1
    assert result["created"] == 0
    assert result["errors"] == 0
    assert result["empty"] == 0
...
async def test_document_management_via_db(db: StandardDatabase, col: StandardCollection):
