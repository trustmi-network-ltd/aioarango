...
async def test_vertex_collection_management(db: StandardDatabase, graph: Graph, bad_graph: Graph):
...
async def test_edge_definition_management(db: StandardDatabase, graph: Graph, bad_graph: Graph):
    ecol_name = generate_col_name()
...
async def test_vertex_management(fvcol: VertexCollection, bad_fvcol: VertexCollection, fvdocs):
    ...
    assert result["_key"] == key
    assert "_rev" in result
    assert await fvcol.has(vertex, check_rev=False) and await fvcol.has(key, check_rev=False)
    ...
    result = await fvcol.update({"_key": key, "foo": 100}, return_old=True, return_new=True)
    ...
    result = await fvcol.replace({"_key": key, "baz": 100}, return_old=True, return_new=True)
    assert "old" in result
    assert "new" in result
    assert "vertex" in result
...
async def test_edge_management(
    ...
    assert result["_key"] == key
    assert "_rev" in result
    assert await ecol.has(edge, check_rev=False) and await ecol.has(key, check_rev=False)
    assert await ecol.count() == 1
    ...
    result = await ecol.update({"_key": key, "foo": 100}, return_old=True, return_new=True)
    assert "old" in result
    assert "new" in result
...
async def test_traverse(db: StandardDatabase):
    ...
    # Insert test edges into the graph
    await teaches.insert({"_from": f"{profs.name}/anna", "_to": f"{classes.name}/CSC101"})
    await teaches.insert({"_from": f"{profs.name}/anna", "_to": f"{classes.name}/STA201"})
    await teaches.insert({"_from": f"{profs.name}/anna", "_to": f"{classes.name}/MAT223"})
    await teaches.insert({"_from": f"{profs.name}/andy", "_to": f"{classes.name}/MAT101"})
    await teaches.insert({"_from": f"{profs.name}/andy", "_to": f"{classes.name}/MAT102"})
    await teaches.insert({"_from": f"{profs.name}/andy", "_to": f"{classes.name}/MAT223"})
